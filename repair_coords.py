...
if __name__ == '__main__':
    for t in 'TPOLJH':
        for n in [1, 2, 3]:
            ...
            target = np.load(f'dane/nowe/root/targety_{v}_magisterka.npy')
            target_coords = np.load(f'dane/nowe/root/coordsy_targetow_{v}_magisterka.npy')
            intsy = np.load(f'dane/nowe/root/intsy_{v}_magisterka.npy')

            # Create a dictionary for quick lookup of target values
            coords_to_target = {tuple(coord[:2]): value + 1 for coord, value in zip(target_coords, target)}
            ...
            files_to_delete = [
                ...
                f'dane/nowe/root/targety_{v}_magisterka.npy',
                f'dane/nowe/root/coordsy_targetow_{v}_magisterka.npy',
            ...
            np.save(f'dane/nowe/root/filtered_intsy_{v}.npy', filtered_intsy)
            np.save(f'dane/nowe/root/filtered_coords_{v}.npy', filtered_coords)
            np.save(f'dane/nowe/root/filtered_targets_{v}.npy', filtered_targets)
